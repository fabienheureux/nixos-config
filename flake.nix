{
  ...
  inputs = {
    ...
    secrets = {
      url = "git+ssh://git@git.deuxfleurs.fr/fabientot/nix-secrets.git";
      flake = false;
    };
    chaotic = {
  ...
  outputs = { self, darwin, home-manager, plasma-manager, nixpkgs, flake-utils, disko, agenix, secrets, chaotic } @inputs:
    let
